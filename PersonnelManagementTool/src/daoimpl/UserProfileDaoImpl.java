...
//		 }
	...
	public UserProfile accessProfile(int i) {
		...
		try {
			...
			// use prepared statements to avoid SQL Injection
			String query = "SELECT firstname,lastname,email,company,department,title,work_address,work_city,work_state,work_zip FROM profiles WHERE id=?";
			...
			try {
				...
				while (rs.next()) {
					...
					resultUser.setWork_address(work_a);
					resultUser.setWork_city(work_c);
					resultUser.setWork_state(work_s);
					resultUser.setWork_zip(work_z);
					// add resultUser to the list of search results
					searchResults.add(resultUser);
				} // end while
